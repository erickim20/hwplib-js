import type { ControlPageNumberPosition } from "../../../../object/bodytext/control/ControlPageNumberPosition.js";
import type { CtrlHeaderPageNumberPosition } from "../../../../object/bodytext/control/ctrlheader/CtrlHeaderPageNumberPosition.js";
import { HWPTag } from "../../../../object/etc/HWPTag.js";
import type { StreamWriter } from "../../../../util/compoundFile/writer/StreamWriter.js";

/**
 * 쪽 번호 위치 컨트롤을 쓰기 위한 객체
 *
 * @author neolord
 */
export class ForControlPageNumberPosition {
  /**
   * 쪽 번호 위치 컨트롤을 쓴다.
   *
   * @param pnp 쪽 번호 위치 컨트롤
   * @param sw  스트림 라이터
   */
  public static write(pnp: ControlPageNumberPosition, sw: StreamWriter): void {
    ForControlPageNumberPosition.ctrlHeader(pnp.getHeader(), sw);
  }

  /**
   * 쪽 번호 위치 컨트롤의 컨트롤 헤더 레코드를 쓴다.
   *
   * @param h  쪽 번호 위치 컨트롤의 컨트롤 헤더 레코드
   * @param sw 스트림 라이터
   */
  private static ctrlHeader(h: CtrlHeaderPageNumberPosition, sw: StreamWriter): void {
    ForControlPageNumberPosition.recordHeader(sw);
    sw.writeUInt4(h.getCtrlId());

    sw.writeUInt4(h.getProperty().getValue());
    sw.writeUInt2(h.getNumber());
    sw.writeWChar(h.getUserSymbol().getBytes());
    sw.writeWChar(h.getBeforeDecorationLetter().getBytes());
    sw.writeWChar(h.getAfterDecorationLetter().getBytes());
  }

  /**
   * 컨트롤 헤더 레코드의 레코드 헤더를 쓴다.
   *
   * @param sw 스트림 라이터
   */
  private static recordHeader(sw: StreamWriter): void {
    sw.writeRecordHeader(HWPTag.CTRL_HEADER, 16);
  }
}
