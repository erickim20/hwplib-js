 ...
 * @author neolord
 */
export class ForCtrlHeaderSecd {
  /**
   ...
   * @param sr     스트림 리더
   */
  public static read(header: CtrlHeaderSectionDefine, sr: StreamReader): void {
    ...
    if (sr.isEndOfRecord() === false && sr.getFileVersion()!.isOver(5, 0, 1, 2)) {
      header.setDefaultLanguage(sr.readUInt2());
    }

    if (sr.isEndOfRecord()) return;

    ForCtrlHeaderSecd.unknownRestBytes(sr);
  }
